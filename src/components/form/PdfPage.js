  ...
  return (
    <Container key="reportTable" id="reportTable" sx={{ background: "white" }}>
      {selectedObj.map((row, index) => {
        return (
          <>
            <Card className="qrPage" key={row.id} id={row.id} sx={{width:"595",height:"842px"}}>
              <Grid container spacing={0} sx={{ padding: "20px 10px 5px 10px" }}>
              ...
              {Array.from({ length: body }, (_, i) => (
                ...
                >
                  <Grid item xs={12} className="pdfItem" sx={{borderRadius: "5px 5px 0px 0px"}}>
                    {row.guia}
                  </Grid>
