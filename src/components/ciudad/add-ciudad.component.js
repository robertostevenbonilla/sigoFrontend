...
import { AssignmentInd, Dashboard, HolidayVillage } from "@mui/icons-material";
    ...
    CiudadDataService.create(dataAU)
      .then((response) => {
        ...
        if (response.status === 200 || response.status === 201) {
          ...
          dispatch(
            setMessage({ ...message, msg: "Se creo correctamente la ciudad" })
          );
    ...
    <div style={{ width: "100%", margin: "0px auto" }}>
      <Breadcrumbs aria-label="breadcrumb" sx={{ marginBottom: "10px" }}>
        ...
        <Chip
          ...
          sx={{ background: "#3364FF", color: "white", padding: "2px 5px" }}
        />
        <Typography color="text.primary">Nueva ciudad</Typography>
      </Breadcrumbs>
      <Card
        title="Ciudad"
